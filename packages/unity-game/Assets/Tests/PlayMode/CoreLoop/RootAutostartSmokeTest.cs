...
namespace GLD.Tests.PlayMode.CoreLoop
{
    public sealed class RootAutostartSmokeTest
    {
        ...
        public IEnumerator RootAutostartStartsCoreLoop()
        {
            ...
            var sfx = controller.GetComponent<CoreLoopSfxController>();
            Assert.That(sfx, Is.Not.Null);
            Assert.That(sfx.IsBound, Is.True, "core loop SFX should bind on Root load");
            var attackPlayCount = sfx.AttackPlayCount;
            ...
            Assert.That(renderer.ActiveWallArrowFxCount, Is.EqualTo(1), "wall auto attack should start as a medieval arrow projectile");
            Assert.That(sfx.AttackPlayCount, Is.GreaterThan(attackPlayCount), "wall auto attack should play an attack SFX");
            Assert.That(controller.DamageNumbers.ActiveCount, Is.EqualTo(0), "wall launch should not show damage before the arrow impacts");
            GameEvents.RaiseWallProjectileImpacted(new WallProjectileImpactEvent(0f, 0f, 12f));
            Assert.That(controller.DamageNumbers.ActiveCount, Is.GreaterThan(0), "wall projectile impact should show damage text");
            ...
            var wallUpgrade = controller.GetComponent<WallUpgradeOverlayController>();
            Assert.That(wallUpgrade, Is.Not.Null);
            Assert.That(wallUpgrade.IsBound, Is.True, "wall upgrade overlay should bind on Root load");
            ...
            Assert.That(uiDocument.rootVisualElement.Q<VisualElement>("game-hud-actions"), Is.Null);
            Assert.That(uiDocument.rootVisualElement.Q<VisualElement>("game-hud-left-stack"), Is.Null);
            Assert.That(uiDocument.rootVisualElement.Q<VisualElement>("hud-wall-repair"), Is.Null);
            Assert.That(uiDocument.rootVisualElement.Q<Button>("hud-gacha-t2"), Is.Null);
            Assert.That(uiDocument.rootVisualElement.Q<Button>("hud-gacha-t3"), Is.Null);
            Assert.That(uiDocument.rootVisualElement.Q<Button>("hud-gacha-t4"), Is.Null);
            Assert.That(topRight, Is.Not.Null);
            Assert.That(uiDocument.rootVisualElement.Q<Button>("hud-speed"), Is.Null);
            Assert.That(menuButton, Is.Not.Null);
            Assert.That(menuButton.text, Is.EqualTo("☰"));
            ...
            var energyPanel = uiDocument.rootVisualElement.Q<VisualElement>("hud-energy-panel");
            var wavePanel = uiDocument.rootVisualElement.Q<VisualElement>("hud-wave-panel");
            Assert.That(energyPanel, Is.Not.Null);
            Assert.That(wavePanel, Is.Not.Null);
            Assert.That(menuButton.resolvedStyle.borderTopWidth, Is.EqualTo(menuButton.resolvedStyle.borderBottomWidth).Within(0.1f));
            Assert.That(energyPanel.resolvedStyle.borderTopWidth, Is.EqualTo(menuButton.resolvedStyle.borderTopWidth).Within(0.5f));
            Assert.That(wavePanel.resolvedStyle.borderTopWidth, Is.EqualTo(menuButton.resolvedStyle.borderTopWidth).Within(0.5f));
            Assert.That(uiDocument.rootVisualElement.Q<Label>("hud-wall-menu-label"), Is.Null);
            var hitUnit = controller.Units.SpawnById("scout_drone");
            Assert.That(hitUnit, Is.Not.Null);
            yield return null;
            var hitPlayCount = sfx.HitPlayCount;
            controller.Units.ApplyDamage(hitUnit, 1f, armorPierce: true);
            yield return null;
            Assert.That(sfx.HitPlayCount, Is.GreaterThan(hitPlayCount), "monster damage should play a hit SFX");
            Assert.That(renderer.ActiveUnitHitSpriteCount, Is.GreaterThan(0), "monster damage should swap to a hit sprite frame");
            var deathPlayCount = sfx.DeathPlayCount;
            controller.Units.ApplyDamage(hitUnit, hitUnit.MaxHp, armorPierce: true);
            yield return null;
            Assert.That(sfx.DeathPlayCount, Is.GreaterThan(deathPlayCount), "monster death should play a death SFX");
            Assert.That(renderer.ActiveUnitDeathFxCount, Is.GreaterThan(0), "monster death should leave a visible death sprite frame");
            controller.Wall.GrantInstantRepairCharge();
            controller.Wall.TakeDamage(5);
            yield return null;
            Assert.That(wallHpLabel.text, Does.Contain("15/20"));
            Assert.That(wallUpgrade.IsVisible, Is.False);
            GameEvents.RaiseWallSelected();
            yield return null;
            Assert.That(wallUpgrade.IsVisible, Is.True);
            Assert.That(uiDocument.rootVisualElement.Q<Button>("wall-upgrade-damage"), Is.Not.Null);
            wallUpgrade.Hide();
