...
| Unity Phase 5 UI/UX parity | `RunState` observable aggregate + `GameSceneController` event projection 착수. UI Toolkit DS primitives 6종(`GLDButton/Card/Badge/Panel/Overlay/Sheet`) + debug gallery + EditMode smoke 추가. `GameHud.uxml`/`GameHudController` 1차 연결 완료. `TowerActionSheet.uxml`/`TowerActionSheetController` 1차로 탭 선택 표시와 판매 요청을 연결했다. `SummonRevealOverlay.uxml`/`SummonRevealController` 1차로 소환 결과 표시와 취소 요청을 연결했다. 다음 범위는 UpgradePick/Pause overlay, Lobby/MetaForge screen | 🔄 |
