...
> **Last Updated:** 2026-05-05 (v3.3 — Unity Phase 2 PoC vertical slice)
>
...
| `unity-game` | Unity 6 WebGL 포트. Phase 2는 `/unity/?slice=poc` PoC slice만 제공 | Unity 6, `@gld/shared` JSON export |
...
`unity-game` Phase 2는 PoC vertical slice다. `Assets/Scripts/Systems/Minimal/`의 pure C# systems가 1 archer + 5 wave-1 scout units + placement + energy/HUD만 처리하며, `Assets/Scripts/SceneRuntime/Slice2/`가 scene glue와 `/unity/?slice=poc` additive routing을 담당한다. Phaser `request-*` EventBus는 이 PoC에 이식하지 않고, Phase 3 full event surface 전까지 `MinimalGameEvents`만 사용한다.
