...
> **GDD 정렬 완료 (2026-04-05):** `docs/2026-04-03-gdd-filled-draft.md`를 실제 구현과 일치시킴 — 웨이브 20→10, 랜덤 타워→4타워 덱 선택 배치, 비용 gold 50→energy 10/20, 그리드 12×8→8×18 세로 모드, 원소 표기 정리 (ice→water 해당 없음, 문서 내 ice/nature/dark 미존재 확인)
...
## MVP 1차 개선 — ✅ 완료 (2026-04-07)

| 항목 | 상태 | 비고 |
|------|------|------|
| 시작 에너지 10 | ✅ | `INITIAL_ENERGY=10` |
| 킬 에너지 보상 차등 | ✅ | 일반 +2, 보스 +5 (`EnergySystem.add()`) |
| 보스 HP바 위치 수정 | ✅ | 게임 캔버스 위 HUD 영역으로 이동 |
| 2배속 토글 | ✅ | 맵 첫 클리어 후 해금, `request-set-speed` EventBus |
| 게임 종료 화면 개선 | ✅ | 픽셀 배너 + 스탯 그리드 + bounce 애니메이션 |
| 맵별 골드/XP 배율 | ✅ | 숲 1x / 용암 2x / 폭풍 3x |
| 난이도 5배 | ✅ | 전 맵 유닛 HP `difficultyHpMult=5` |
| wavesCleared 버그픽스 | ✅ | 패배 시 finalSlot-1 |

테스트: 139개 PASS (phaser-game 기준)
