import type { Position, PressurePacketId, TowerDef } from '@gld/shared';
import {
	EMOTES,
	FOREST_GATE_MAP,
...
export class AIOpponent {
	...
	private buyCooldownMs = 0;
	private pressureInventory: PressurePacketId[] = [];
	private queuedPressure = new Map<number, PressurePacketId>();
	...
	get pressureTokens(): number {
		return this.pressureInventory.length;
	}
	get queuedPressureSlots(): number[] {
		return Array.from(this.queuedPressure.keys()).sort((a, b) => a - b);
	}
	...
	updateRealtime(
		delta: number,
		context: { slotIndex: number; pressureTier?: 1 | 2 | 3 | null },
	): void {
		this.buyCooldownMs = Math.max(0, this.buyCooldownMs - delta);

		if (this.buyCooldownMs <= 0 && this.gold >= RANDOM_TOWER_COST) {
			const tower = this.randomTowerSystem.rollRandomTower();
			const pos = this.findRandomPlacement();
			if (pos) {
				this.gold -= RANDOM_TOWER_COST;
				this.placeTower(pos.x, pos.y, tower);
				this.buyCooldownMs = 1800;
			}
		}

		if (Math.random() < 0.1 && this.towers.length >= 2) {
			this.tryMerge();
		}

		if (context.pressureTier) {
			this.earnPressure(PRESSURE_PACKET_BY_TIER[context.pressureTier].id);
		}

		this.reserveNextPressure(context.slotIndex);
		this.emitState();
	}

	earnPressure(packetId: PressurePacketId): boolean {
		if (this.pressureInventory.length >= PRESSURE_TOKEN_CAP) return false;
		this.pressureInventory.push(packetId);
		return true;
	}

	reserveNextPressure(fromSlotIndex: number): number | null {
		if (this.pressureInventory.length === 0) return null;

		let targetSlot = getNextEligiblePressureSlot(fromSlotIndex);
		while (targetSlot && this.queuedPressure.has(targetSlot.slotIndex)) {
			targetSlot = getNextEligiblePressureSlot(targetSlot.slotIndex);
		}
		if (!targetSlot) return null;

		const packetId = this.pressureInventory.shift();
		if (!packetId) return null;
		this.queuedPressure.set(targetSlot.slotIndex, packetId);
	...
	private findRandomPlacement(): Position | null {
		const candidates = FOREST_GATE_MAP.placementPoints.filter((p) =>
			this.gridManager.isWalkable(p.x, p.y),
		);
	...
	private tryMerge(): void {
		for (let i = 0; i < this.towers.length; i++) {
			for (let j = i + 1; j < this.towers.length; j++) {
				if (
					this.towers[i].defId === this.towers[j].defId &&
					this.towers[i].def.tier < 5
				) {
					const nextTier = this.towers[i].def.tier + 1;
					const nextTierTowers = getTowersByTier(nextTier);
					const rolledTower =
						nextTierTowers[Math.floor(Math.random() * nextTierTowers.length)];

					// Remove both towers from grid, then re-place merged tower
					const posJ = this.towers[j].position;
					this.gridManager.removeTower(
						this.towers[i].position.x,
						this.towers[i].position.y,
					);
	...
	update(
		time: number,
		delta: number,
	): { reachedExit: number; killedUnits: string[] } {
	...
	private emitState(): void {
		if (
			this.gold === this.lastEmittedGold &&
			this.hp === this.lastEmittedHp &&
			this.towerCount === this.lastEmittedTowerCount
		)
			return;
