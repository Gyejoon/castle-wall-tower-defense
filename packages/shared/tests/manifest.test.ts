...
describe('inferAssetManifestSection — new sections', () => {
  ...
  it('preserves existing routing unchanged', () => {
    ...
    expect(inferAssetManifestSection({ key: 'tower-laser-fire', path: 'assets/towers/laser-fire.png' })).toBe('vfx');
    expect(inferAssetManifestSection({ key: 'icon-192', path: 'assets/icons/icon-192.png' })).toBe('icons');
    expect(inferAssetManifestSection({ key: 'proj-laser', path: 'assets/projectiles/laser.png' })).toBe('projectiles');
    expect(inferAssetManifestSection({ key: 'lobby-bg', path: 'assets/ui-mobile/lobby.webp' })).toBe('mobile');
  });
});
