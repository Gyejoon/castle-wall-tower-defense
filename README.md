...
모바일 우선 PVE 타워디펜스 버티컬 슬라이스. 에너지 기반 타워 롤과 합성으로 10웨이브 솔로 생존을 벌인다. Phaser 3 + React 프론트엔드, bun 모노레포.

## 현재 빌드에서 할 수 있는 것

- **월드맵 + 스테이지 선택** — 3개 맵(숲·용암·폭풍) 진행도 기반 잠금 해제, 스테이지별 난이도·보상 표시
- **덱 편집** — 출전 타워 라인업 관리, 보유 타워 컬렉션 확인
- **PVE 생존** — 에너지로 타워 롤, 배치, 합성하며 10웨이브 방어
- **에너지 경제** — 시작 에너지 10, 초당 1 축적, 타워 롤에 10 소비. 일반 킬 +2, 보스 킬 +5
- **타워 판매** — 배치된 타워 탭 → 판매 패널 → 에너지 50% 환급
- **타워 합성** — 동일 타워 2개를 다음 티어 랜덤으로 합성 (5티어까지), 드래그 합성 지원
- **물리 충돌** — 지상 유닛 간 분리(MIN_SEPARATION 0.8 tiles), 스턴/슬로우 체이닝, 비행 유닛 면제
- **웨이브 스케일링** — 맵별 아키타입 테마 배치(Scout, Speed, Tank, Stealth, Boss), HP 1.0x~3.5x 자동 스케일링
- **맵별 난이도** — forest_gate 1.0x, lava_fortress 1.3x, storm_citadel 1.6x HP 배수
- **보스 리크** — 보스가 경로 끝 도달 시 즉시 패배
...
| 0+ | 전투 확장 — 타워 판매, 물리 충돌, 웨이브 스케일링, 보스 리크, 나가기 모달, 맵별 난이도 | **완료** |
| 0+ | 월드맵/스테이지 — 3맵 선택, 스테이지 상세, 덱 편집 | **진행 중** |
| 1 | 핵심 전투 — 속성 시스템 전투 적용 (플레이테스트 후 결정) | 계획 |
| 2 | 메타 성장 — 저장, 타워 강화/승급(LV.50 캡), 컬렉션 | 계획 |
| 3 | 참여 시스템 — 튜토리얼, 가챠, 미션 | 계획 |
