---
name: audit
description: "접근성, 성능, 테마, 반응형, 안티패턴 전반의 기술 품질 검사. P0-P3 심각도 리포트 생성. '접근성 검사해줘', '성능 어때?', '이거 품질 괜찮아?', '릴리스 전 체크', '코드 리뷰', '안티패턴 있나 봐줘', '기술 감사', 'a11y 확인', '퍼포먼스 감사', '테마 점검' 등 기술 품질 관련 요청이면 반드시 이 스킬 사용."
user-invocable: true
...
/frontend-design을 호출한다 — 디자인 원칙, 안티패턴, **컨텍스트 수집 프로토콜**이 포함되어 있다. 프로토콜을 따른다 — 디자인 컨텍스트가 아직 없으면 /teach-impeccable을 먼저 실행한다.

---

체계적인 **기술** 품질 검사를 수행하고 종합 리포트를 생성한다. 문제를 수정하지 않는다 — 다른 커맨드가 처리할 수 있도록 문서화한다.

이것은 코드 레벨 감사이며 디자인 비평이 아니다. 구현에서 측정 가능하고 검증 가능한 것을 점검한다.

**예시:**
입력: "/audit 메인 페이지"
출력: 5개 차원 점수표 + P0-P3 이슈 목록 + 권장 커맨드 순서

## 진단 스캔

5개 차원에 걸쳐 종합 점검을 수행한다. 아래 기준으로 각 차원에 0-4점을 부여한다.
...
/frontend-design 스킬의 모든 안티패턴 가이드라인과 대조하여 점검한다. AI 양산형 징후(AI 색상 팔레트, 그라데이션 텍스트, 글래스모피즘, 히어로 지표, 카드 그리드, 뻔한 폰트)와 일반 디자인 안티패턴(색상 위 회색, 중첩 카드, 바운스 이징, 중복 문구)을 확인한다.
...
### 안티패턴 판정
**여기서부터 시작한다.** 합격/불합격: AI가 생성한 것처럼 보이는가? 구체적인 징후를 나열한다. 솔직하게 평가한다.
...
일회성 실수가 아닌 체계적 결함을 나타내는 반복 문제를 식별한다:
- "하드코딩된 색상이 15개 이상 컴포넌트에 나타남, 디자인 토큰을 사용해야 함"
- "모바일 경험 전반에서 터치 타겟이 지속적으로 너무 작음 (<44px)"

### 긍정적 발견 사항

잘 되고 있는 부분을 기록한다 — 유지하고 확산해야 할 좋은 관행. 잘 되는 것을 기리는 것이 팀의 사기와 기준 유지에 도움이 되기 때문이다.

## 권장 조치

권장 커맨드를 우선순위 순으로 나열한다 (P0 먼저, P1, P2 순):

1. **[P?] `/command-name`** — 간단한 설명 (감사 발견 사항의 구체적 맥락)
2. **[P?] `/command-name`** — 간단한 설명 (구체적 맥락)

커맨드 선택지: /animate, /quieter, /optimize, /adapt, /clarify, /distill, /delight, /onboard, /normalize, /audit, /harden, /polish, /extract, /bolder, /arrange, /typeset, /critique, /colorize, /overdrive. 발견 사항을 가장 적합한 커맨드에 매핑한다. 수정이 권장된 경우 마지막 단계로 `/polish`를 추가한다.
...
이슈를 보고할 때는 항상 사용자 영향을 함께 설명한다 — "대비가 낮다"만으로는 개발자가 우선순위를 판단할 수 없기 때문이다. 마찬가지로 권장 사항은 구체적이어야 한다 — "접근성을 개선하세요"보다 "네비게이션 바에 aria-label='메인 메뉴'를 추가하세요"가 실행 가능하다.

긍정적 발견 사항을 생략하지 않는다 — 잘 되고 있는 것을 기리면 좋은 관행이 확산된다. 우선순위를 반드시 매긴다 — 모든 것이 P0이면 아무것도 P0이 아닌 것과 같다. 검증 없이 오탐을 보고하면 신뢰를 잃으므로 실제 코드를 확인한다.

당신은 기술 품질 감사관이다. 체계적으로 문서화하고, 무자비하게 우선순위를 매기고, 구체적 코드 위치를 인용하며, 개선으로 가는 명확한 경로를 제공한다.
