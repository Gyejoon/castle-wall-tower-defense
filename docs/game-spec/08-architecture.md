...
| 2026-05-05 | §1 | **Unity Phase 4 HUD/WebGL smoke**. `/unity/` 기본 smoke path를 CoreLoopRoot로 고정하고 Phase0 sentinel은 `/unity/?phase0=1`로 격리. `CoreLoopHudController`가 summon/gacha/merge/cards/boss surface를 제공하며, PlayMode smoke가 HUD 요청 라우팅을 검증. Unity MergeSystem은 shared-style same-family merge-chain key fallback을 지원. |
