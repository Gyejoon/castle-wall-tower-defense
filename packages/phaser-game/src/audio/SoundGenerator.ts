...
export class SoundGenerator {
  ...
  private playFM(
    ...
    if (modDepthEnd !== undefined) {
      modGain.gain.linearRampToValueAtTime(modDepthEnd, ctx.currentTime + duration / 1000);
    }

    outputGain.gain.setValueAtTime(volume, ctx.currentTime);
    outputGain.gain.linearRampToValueAtTime(0, ctx.currentTime + duration / 1000);
  ...
  playTowerUpgraded(): void {
    ...
    for (const note of notes) {
      setTimeout(() => {
        this.play({ frequency: note.frequency, duration: 80, type: 'square', volume: 0.12 });
      }, note.delay);
    }
  }
  ...
  playWaveComplete(): void {
    ...
    for (const note of notes) {
      setTimeout(() => {
        this.play({ frequency: note.frequency, duration: 120, type: 'sine', volume: 0.15 });
      }, note.delay);
    }
  ...
  playUnitSpawned(): void {
    this.playLayered([
      ...
      {
        kind: 'osc',
        recipe: { frequency: 800, duration: 40, type: 'sine', volume: 0.10 },
      },
    ]);
  }
  ...
  playBreach(): void {
    this.playLayered([
      {
        kind: 'osc',
        recipe: { frequency: 300, endFrequency: 350, duration: 200, type: 'square', volume: 0.18 },
      },
      {
        kind: 'noise',
        config: {
