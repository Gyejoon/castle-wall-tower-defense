...
describe('towerToRole', () => {
	...
	it('categorizes all 18 towers without error', () => {
		for (const tower of ALL_TOWERS) {
			expect(['attacker', 'splash', 'slow', 'stun']).toContain(towerToRole(tower));
		}
	});
});
...
describe('buildDeckCards', () => {
	it('builds 4 DeckCardDef from valid tower IDs', () => {
		...
		expect(cards[0]).toEqual({ towerDefId: 'laser', energyCost: 10, role: 'attacker' });
		expect(cards[2]).toEqual({ towerDefId: 'emp', energyCost: 20, role: 'slow' });
	});
	it('works with higher tier towers', () => {
		const cards = buildDeckCards(['dragon_nest', 'disruptor', 'holy_shrine', 'wind_spire']);
		expect(cards[0].energyCost).toBe(10);
		expect(cards[1].role).toBe('slow');
		expect(cards[2].role).toBe('stun');
	});
	it('throws on unknown tower ID', () => {
		expect(() => buildDeckCards(['nonexistent', 'laser', 'plasma', 'emp'])).toThrow();
	});
});
