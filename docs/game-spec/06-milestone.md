...
| Unity Phase 5 UI/UX parity | `RunState` observable aggregate + `GameSceneController` event projection 착수. UI Toolkit DS primitives 6종(`GLDButton/Card/Badge/Panel/Overlay/Sheet`) + debug gallery + EditMode smoke 추가. `GameHud.uxml`/`GameHudController` 1차 연결 완료. `TowerActionSheet.uxml`/`TowerActionSheetController` 1차로 탭 선택 표시와 판매 요청을 연결했다. `SummonRevealOverlay.uxml`/`SummonRevealController` 1차로 소환 결과 표시와 취소 요청을 연결했다. `UpgradePickOverlay.uxml`/`UpgradePickOverlayController`가 3카드 선택/리롤 요청을 처리하고, `PauseModal.uxml`/`PauseModalController`가 `RunState.IsPaused` 표시와 resume/quit 요청을 처리한다. `BossHpBar.uxml`/`BossHpBarController`와 `BossWarningOverlay.uxml`/`BossWarningOverlayController`가 보스 HP와 보스 웨이브 경고 표시를 처리한다. `GameOverOverlay.uxml`/`GameOverOverlayController`가 승리/패배 결과와 런 요약, quit 요청을 처리한다. `ToastOverlay.uxml`/`ToastOverlayController`가 거절/배치/합성/강화 피드백을 처리한다. `TutorialOverlay.uxml`/`TutorialOverlayController`가 소환→배치→합성→강화 선택 안내를 처리한다. `LobbyMetaScreen.uxml`/`LobbyMetaScreenController`가 초기 Home/Meta Forge shell과 start-run 요청을 처리한다. 다음 범위는 Unity continue/save persistence, real AdService, shell polish | 🔄 |
