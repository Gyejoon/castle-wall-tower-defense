...
| `unity-game` | Unity 6 WebGL 포트. Phase 2 PoC를 유지하면서 Phase 3 1차 코어를 닫고 Phase 4 인런 parity를 진행 중 | Unity 6, `@gld/shared` JSON export |
...
`unity-game` Phase 2 PoC는 계속 보존한다. `Assets/Scripts/Systems/Minimal/`의 pure C# systems가 1 archer + 5 wave-1 scout units + placement + energy/HUD만 처리하며, `Assets/Scripts/SceneRuntime/Slice2/`가 scene glue와 `/unity/?slice=poc` additive routing을 담당한다. Phase 3 코어는 별도 `Assets/Scripts/Systems/{Grid,Pathfinding,Energy,Units,Waves,Towers,DamageNumbers,Orchestrator}`와 `Assets/Scripts/SceneRuntime/CoreLoop/`에 병렬 추가되어, RNG/Grid/Pathfinding/Energy/기본 Unit-Wave-Tower loop를 검증한다. Phase 4는 이 코어 위에 `Systems/{Merge,Gacha,Upgrade,Boss}`를 추가해 Phaser의 현재 merge/gacha/roguelike/boss behavior parity를 닫는다. `CoreLoopFieldRenderer`가 9×18 grid/path/buildable/tower/unit 상태를 SpriteRenderer 기반으로 표시하고, `CoreLoopHudController`가 IMGUI 기반 energy/wave/status HUD와 summon/gacha tier 버튼, direct archer placement, merge mode, upgrade card choice, boss HP/phase surface를 제공한다. `GameEvents`는 typed request/state 이벤트 표면을 제공하고, `CoreOrchestrator`는 summon/placement/sell/move/merge/gacha/upgrade 및 cancelled pool/gacha draw cache를 처리한다. Unity `MergeSystem`은 SO의 direct target이 비어 있어도 shared-style `archer_1_same` merge-chain key를 fallback으로 해석한다. `packages/shared/src/testing/replay-fixtures/seed-001..010`과 `replay-runner.ts`가 TS reference CSV를 생성하며, Phase 4에서는 seed-002/003/004를 ungate하고 continue/tutorial fixtures는 후속 Phase까지 skipped로 유지한다. `Tests/PlayMode/Integration/FullRunFixtureTest.cs`와 `BossEncounterTest.cs`가 local gate로 full-run in-run systems와 4 boss phase/death cycle을 검증한다. `.github/workflows/unity-parity-gate.yml`이 Unity EditMode parity test를 실행한다. `/unity/` smoke path는 기본적으로 CoreLoopRoot를 표시하고, `/unity/?autostart=1`은 즉시 wave 1을 시작한다. Phase 0 sentinel은 `/unity/?phase0=1`로만 남긴다. Full HUD polish, real AdService, save persistence는 후속 Phase 범위다.
