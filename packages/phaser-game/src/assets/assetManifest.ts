...
export const OPTIONAL_ASSET_SECTIONS: AssetManifestSection[] = ['ui', 'vfx', 'projectiles'];
...
export function unloadAssetSections(
  ...
  for (const asset of dedupeAssets(sections.flatMap((section) => getManifestSectionEntries(manifest, section)))) {
    switch (asset.type) {
      case 'image':
      case 'spritesheet':
  ...
  switch (asset.type) {
    ...
    case 'spritesheet':
      if (!asset.frameWidth || !asset.frameHeight) {
        throw new Error(`spritesheet asset is missing frame metadata: ${asset.key}`);
      }
...
function isAssetLoaded(scene: Phaser.Scene, asset: AssetManifestEntry): boolean {
  switch (asset.type) {
    case 'image':
    case 'spritesheet':
