/**
 * AI Tile Generation via ComfyUI
 *
 * Generates 4 tile types using Stable Diffusion with ControlNet Tile.
 *
...
import { postProcessImage } from './ai-post-process';
import type { ManifestEntry } from './shared';

function buildTileWorkflow(config: AssetPromptConfig): ComfyUIWorkflow {
  return {
    '3': {
      class_type: 'KSampler',
      inputs: {
    ...
    '5': {
      ...
      inputs: { width: COMFYUI_CONFIG.width, height: COMFYUI_CONFIG.height, batch_size: 1 },
    },
    '6': {
    ...
    '9': {
      ...
      inputs: { filename_prefix: `ai-tile-${config.key}`, images: ['8', 0] },
    },
  };
}

async function generateTile(config: AssetPromptConfig): Promise<ManifestEntry> {
  console.log(`  generating tile: ${config.key}`);

  const tempPath = `${AI_TEMP_DIR}/${config.key}-raw.png`;
  mkdirSync(dirname(tempPath), { recursive: true });

  const promptId = await queuePrompt(buildTileWorkflow(config));
  ...
  if (outputs.length === 0 || outputs[0].images.length === 0) {
    throw new Error(`No output images for tile ${config.key}`);
  }

  const image = outputs[0].images[0];
  await downloadImage(image.filename, image.subfolder, tempPath);

  await postProcessImage(tempPath, config.outputPath, {
...
export async function generate(): Promise<ManifestEntry[]> {
  console.log('Generating AI tiles via ComfyUI...');

  if (!(await checkAvailable())) {
    console.warn('  WARNING: ComfyUI not available. Skipping AI tile generation.');
  ...
  const results = await Promise.allSettled(
    TILE_PROMPTS.map((config) => generateTile(config)),
  );
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (result.status === 'fulfilled') {
      ...
      console.error(`  ERROR generating tile ${TILE_PROMPTS[i].key}:`, result.reason);
    }
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  generate()
    .then((entries) => console.log(`\nGenerated ${entries.length} tiles`))
