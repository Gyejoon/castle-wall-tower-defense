...
> **Last Updated:** 2026-05-06 (v3.24 — Unity WebGL Vercel preview)
>
...
| `unity-game` | Unity 6 WebGL 포트. Phase 2 PoC를 유지하면서 Phase 3 1차 코어와 Phase 4 인런 parity를 완료 | Unity 6, `@gld/shared` JSON export |

`web-shell`은 `@gld/phaser-game`을 import하지만, 런타임 통신은 EventBus를 통해서만 한다. 직접 시스템 인스턴스를 참조하지 않는다.

개발용 tools-app은 제거됐다. 에셋/타일맵/씬/밸런스 관리용 로컬 Vite middleware와 `bun run dev:tools` 진입점은 더 이상 유지하지 않는다.

`unity-game` Phase 2 PoC는 계속 보존한다. `Assets/Scripts/Systems/Minimal/`의 pure C# systems가 1 archer + 5 wave-1 scout units + placement + energy/HUD만 처리하며, `Assets/Scripts/SceneRuntime/Slice2/`가 scene glue와 `/unity/?slice=poc` additive routing을 담당한다. Phase 3 코어는 별도 `Assets/Scripts/Systems/{Grid,Pathfinding,Energy,Units,Waves,Towers,DamageNumbers,Orchestrator}`와 `Assets/Scripts/SceneRuntime/CoreLoop/`에 병렬 추가되어, RNG/Grid/Pathfinding/Energy/기본 Unit-Wave-Tower loop를 검증한다. Unity `WaveSystem`은 Phaser와 같이 일반 웨이브가 30초를 넘으면 잔존 유닛이 있어도 다음 웨이브를 즉시 시작하며, 보스/최종 웨이브는 타임아웃 강제 진행에서 제외한다. Phase 4는 이 코어 위에 `Systems/{Merge,Gacha,Upgrade,Boss}`를 추가해 Phaser의 현재 merge/gacha/roguelike/boss behavior parity를 닫았다. Phase 5는 UI Toolkit 전환 기반으로 `RunState` observable aggregate를 추가하고, `GameSceneController`가 energy/wave/boss/HP/speed/pause/game-over 이벤트를 UI snapshot으로 투영하기 시작했다. `Assets/Scripts/UI/Primitives/`에는 `GLDButton/Card/Badge/Panel/Overlay/Sheet` 6종이 추가되어 UXML trait 기반 variant/tier/element class를 제공한다. `Assets/Scripts/SceneRuntime/CoreLoop/UI/GameHudController.cs`는 CoreLoopRoot에 `UIDocument`를 붙여 `RunState`를 `GameHud.uxml`/fallback HUD로 바인딩하고 summon/gacha/menu request를 `GameEvents`로 발행한다. `RuntimeUiDocument`는 `Resources/UI/GameHudPanelSettings.asset`를 로드해 CoreLoop UI의 PanelSettings/theme을 보장한다. `TowerActionSheetController.cs`는 같은 `UIDocument`에 `TowerActionSheet.uxml`/fallback sheet를 붙이고, `InputController`의 타워 탭 선택 이벤트를 받아 선택 타워 표시와 판매 요청을 발행한다. 이동/합성 버튼은 타깃 선택 플로우가 완성될 때까지 비활성으로 둔다. `SummonRevealController.cs`는 `SummonRevealOverlay.uxml`/fallback toast를 붙이고 `OnSummonOffered/Cancelled/Confirmed`를 따라 소환 결과와 취소 요청을 표시한다. `UpgradePickOverlayController.cs`는 `UpgradePickOverlay.uxml`/fallback overlay를 붙이고 `OnUpgradeChoiceReady/Applied`를 따라 3카드 선택과 리롤 요청을 처리한다. `PauseModalController.cs`는 `PauseModal.uxml`/fallback modal을 붙이고 `RunState.IsPaused`에 따라 표시되며 `request-resume`과 `request-quit-to-lobby` 요청을 발행한다. `BossHpBarController.cs`는 `BossHpBar.uxml`/fallback bar를 붙이고 `RunState.Boss*` 상태에 따라 boss HP bar를 표시한다. `BossWarningOverlayController.cs`는 `BossWarningOverlay.uxml`/fallback overlay를 붙이고 `OnBossWaveStarted` 이벤트에 따라 일시 경고를 표시한다. `GameOverOverlayController.cs`는 `GameOverOverlay.uxml`/fallback overlay를 붙이고 `RunState.RunStatus`가 `Victory`/`Defeat`일 때 결과, wave/HP/time 요약, quit 요청을 표시한다. `ToastOverlayController.cs`는 `ToastOverlay.uxml`/fallback panel을 붙이고 request rejected, placement failed, merge failed/success, upgrade applied 이벤트를 짧은 피드백으로 표시한다. `TutorialOverlayController.cs`는 `TutorialOverlay.uxml`/fallback panel을 붙이고 summon offered/confirmed, merge success, upgrade choice/applied 이벤트에 따라 소환→배치→합성→강화 선택 안내를 전진하거나 닫는다. `LobbyMetaScreenController.cs`는 `LobbyMetaScreen.uxml`/fallback overlay를 바인딩하되 CoreLoop 기본 진입에서는 `ShellEnabled=false`로 숨긴다. 로비/메타 셸 검증이나 별도 화면이 필요할 때만 `ShellEnabled`를 켠다. `CoreLoopFieldRenderer`는 `main_long`의 `main-long-bg.png`를 전체 맵 배경 SpriteRenderer로 깔고, 그 위에 런타임 `Grid`/`Tilemap` 2층(`TerrainTilemap`, `BuildableOverlayTilemap`)을 만든다. `main_long`에서는 불필요한 타일 에셋 노출을 막기 위해 두 TilemapRenderer를 기본 비활성화하고, 필요할 때 `showMapTileLayers`로만 켠다. 타워/유닛/배치 마커는 SpriteRenderer로 유지하며, 판정 정본은 PNG나 Tilemap이 아니라 `MapLayoutSO`와 `GridManager`다. `GLD/Map Runtime Layout`/`GLD/Map Placement Anchors` 에디터 창은 `MapLayoutSO.placementAnchors`와 monster `lanes` 웨이포인트를 Scene View에서 드래그 편집하며, Unity 월드 좌표를 shared map pixel 좌표(`worldX`/`worldY`) 또는 float grid 좌표로 되돌려 저장한다. 단, `GLD/Import Shared Data`를 다시 실행하면 `Assets/Resources/GameData/maps.json` 값으로 `MapLayout.asset`이 재생성된다. `CoreLoopHudController`는 기본 상태에서 화면 하단 compact IMGUI action bar(`소환`/T2/T3/T4/`합성`/`시작`)만 렌더하고, `showDebugHud`가 켜진 경우에만 기존 대형 개발 HUD를 렌더한다. Unity `Root.unity`와 `ProjectSettings/EditorSettings.asset`은 2D 기본 모드로 저장한다. Main Camera는 orthographic + solid color이며, Directional Light는 비활성으로 둔다. `GameEvents`는 typed request/state 이벤트 표면을 제공하고, `CoreOrchestrator`는 summon/placement/sell/move/merge/gacha/upgrade 및 cancelled pool/gacha draw cache를 처리한다. Unity `MergeSystem`은 SO의 direct target이 비어 있어도 shared-style `archer_1_same` merge-chain key를 fallback으로 해석한다. `packages/shared/src/testing/replay-fixtures/seed-001..010`과 `replay-runner.ts`가 TS reference CSV를 생성하며, Phase 4에서는 seed-002/003/004를 ungate하고 continue/tutorial fixtures는 후속 Phase까지 skipped로 유지한다. `Tests/PlayMode/Integration/FullRunFixtureTest.cs`와 `BossEncounterTest.cs`가 local gate로 full-run in-run systems와 4 boss phase/death cycle을 검증한다. `.github/workflows/unity-parity-gate.yml`은 GitHub에서 TS replay ledger를 통과했으며, hosted Unity EditMode step은 Unity license secrets 미설정 환경에서 스킵된다. `/unity/` smoke path는 CoreLoopRoot만 표시하고, `/unity/?autostart=1`은 즉시 wave 1을 시작한다. `packages/unity-game/Build/WebGL`의 WebGL snapshot은 Vercel preview 확인용으로 커밋되며, `scripts/merge-build.ts`가 preview build 중 `packages/web-shell/dist/unity`로 복사한다. Vercel preview snapshot은 CDN `Content-Encoding` 설정에 의존하지 않도록 uncompressed `WebGL.data`/`WebGL.framework.js`/`WebGL.wasm`를 사용한다. Phase 0 sentinel scene path와 `Phase0Root` 전용 UI는 제거됐다. Full HUD polish, real AdService, save persistence는 후속 Phase 범위다.
...
| 2026-05-05 | §1 | **Unity Phase 4 HUD/WebGL smoke**. `/unity/` 기본 smoke path를 CoreLoopRoot로 고정하고 `CoreLoopHudController`가 summon/gacha/merge/cards/boss surface를 제공하며, PlayMode smoke가 HUD 요청 라우팅을 검증. Unity MergeSystem은 shared-style same-family merge-chain key fallback을 지원. |
| 2026-05-05 | §1 | **Unity Phase 4 parity gate close**. PlayMode `FullRunFixtureTest`/`BossEncounterTest`와 EditMode replay/gacha/boss parity를 로컬 통과 기준으로 닫고, GitHub `Unity Parity Gate` run `25379788767`에서 TS replay ledger를 통과. GitHub hosted Unity step은 license secret 미설정으로 스킵됨을 명시. |
| 2026-05-05 | §1 | **Unity Phase 5 UI/UX parity start**. `RunState` pure C# observable snapshot을 추가하고 `GameStateManager`/`GameSceneController`에서 HP, energy, wave, countdown, boss, speed, pause, game-over 상태를 발행하도록 연결. |
| 2026-05-06 | §1 | **Unity Phase 5 DS primitives**. `GLDButton/Card/Badge/Panel/Overlay/Sheet` 6종, `primitives.uss`, `_DebugPrimitivesGallery.uxml`, `GLDPrimitivesTest`를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 GameHud 1차**. `GameHud.uxml`, `hud.uss`, `GameHudController`, EditMode label/request tests, Root PlayMode smoke binding check를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 TowerActionSheet 1차**. `TowerActionSheet.uxml`, `overlays.uss`, `TowerActionSheetController`, 탭 선택/해제 이벤트, 판매 request 연결, EditMode binding/request tests, Root PlayMode smoke binding check를 추가. |
| 2026-05-06 | §1 | **Unity Phase 0 sentinel 제거**. `Root.unity`의 `Phase0Root`, `GLDPhase0Label`, `Phase0Label.uxml`, `phase0.uss`, `PhaseZeroPanelSettings`, `phase0=1` 라우팅을 제거하고 `/unity/` 진입점을 CoreLoop 전용으로 단순화. |
| 2026-05-06 | §1 | **Unity Phase 5 SummonReveal 1차**. `SummonRevealOverlay.uxml`, `SummonRevealController`, toast 스타일, 취소 request 연결, EditMode binding/request tests, Root PlayMode smoke binding check를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 UpgradePick/Pause 1차**. `UpgradePickOverlay.uxml`, `PauseModal.uxml`, 각 controller, EditMode request/visibility tests, Root PlayMode smoke binding check를 추가. `CoreLoopHudController`는 카드 렌더를 중단하고 UI Toolkit overlay에 위임. |
| 2026-05-06 | §1 | **Unity Phase 5 Boss UI 1차**. `BossHpBar.uxml`, `BossWarningOverlay.uxml`, 각 controller, EditMode visibility tests, Root PlayMode smoke binding/event check를 추가. |
| 2026-05-06 | §1 | **Unity wave timer parity fix**. 일반 웨이브 30초 타임아웃 시 잔존 유닛이 있어도 다음 웨이브를 즉시 시작하도록 `WaveSystem`을 Phaser 규칙과 맞추고, wave2 스폰 회귀 테스트를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 GameOver overlay 1차**. `GameOverOverlay.uxml`, `GameOverOverlayController`, EditMode visibility/request tests, Root PlayMode game-over smoke check를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 Toast overlay 1차**. `ToastOverlay.uxml`, `ToastOverlayController`, EditMode event feedback tests, Root PlayMode toast smoke check를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 Tutorial overlay 1차**. `TutorialOverlay.uxml`, `TutorialOverlayController`, EditMode progression/dismiss tests, Root PlayMode binding smoke check를 추가. |
| 2026-05-06 | §1 | **Unity Phase 5 Lobby/Meta shell 1차**. `LobbyMetaScreen.uxml`, `LobbyMetaScreenController`, EditMode tab/start tests, Root PlayMode initial shell/start hide smoke check를 추가. |
| 2026-05-06 | §1 | **Unity CoreLoop Tilemap 렌더 전환**. `main-long-bg.png`를 배경으로 유지하고 `CoreLoopFieldRenderer`가 런타임 `Grid`/`Tilemap` 2층을 만들도록 변경. `main_long`에서는 타일 에셋 노출을 기본 비활성화하고, 게임 판정은 계속 `MapLayoutSO`/`GridManager`를 정본으로 사용한다. `CoreLoopHudController`는 기본 compact action bar를 유지하고, 대형 IMGUI 개발 HUD만 `showDebugHud` 뒤로 숨겼다. `LobbyMetaScreenController` 셸은 기본 비활성화했다. |
| 2026-05-06 | §1 | **Unity 소환 배치 경로 수정**. `PlacementCoordinator`가 `OnSummonOffered/Cancelled/Confirmed`를 구독해 `InputController` 배치 모드와 소환 pending 상태를 동기화하고, 필드 탭 배치는 `request-place-tower`를 통해 `CoreOrchestrator`가 처리하도록 변경했다. |
| 2026-05-06 | §1 | **Unity 시작 전 tick/입력 수정**. CoreLoop는 `Building` 상태에서 5초 카운트다운 후 자동으로 wave 1을 시작하며, `WavePhase.Idle` 동안 energy/tower/unit/wave tick을 실행하지 않는다. `InputController`는 상단/하단 HUD 영역 터치를 필드 배치로 처리하지 않도록 막았다. |
| 2026-05-06 | §1 | **Unity 배치 앵커 드래그 편집 툴 추가**. `GLD/Map Placement Anchors` EditorWindow가 `MapLayoutSO.placementAnchors`를 Scene View 핸들로 편집하고, Unity 월드 좌표를 shared map pixel 좌표(`worldX`/`worldY`)로 변환해 저장한다. |
| 2026-05-06 | §1 | **Unity 배치 앵커 hit-test 수정**. 배치 마커/타워가 `GridToPlacementWorld` 앵커 위치에 그려지는 만큼 입력도 `WorldToPlacementGrid`로 가장 가까운 배치 앵커를 먼저 찾고, 실패할 때만 기존 `WorldToGrid`로 fallback한다. 기본 숨김 상태의 legacy IMGUI HUD는 필드 클릭을 중복 처리하지 않는다. |
| 2026-05-06 | §1 | **Unity 타워 전투 좌표 분리**. 타워 렌더 위치는 `GridToPlacementWorld` 앵커를 유지하되, 사거리/타깃 탐색은 논리 셀 중심(`GridToWorld`) 기준 `CombatPosition`으로 계산한다. 앵커 드래그로 그림 위치를 조정해도 공격 판정이 깨지지 않는다. |
| 2026-05-06 | §1 | **Unity 몬스터 경로 드래그 편집 추가**. `MapPlacementAnchorEditorWindow`에 `Monster Path` 모드를 추가해 `MapLayoutSO.lanes`의 float-grid waypoints를 Scene View에서 드래그 편집한다. 첫 점은 spawn, 마지막 점은 exit로 동기화한다. |
...
| 2026-05-06 | §1 | **Unity WebGL Vercel preview 활성화**. `packages/unity-game/Build/WebGL` snapshot을 커밋 가능한 예외 경로로 열고, Vercel ignore 예외와 WebGL MIME header 설정을 추가해 preview deployment의 `/unity/`에서 실제 Unity WebGL build를 확인할 수 있게 했다. `/unity`는 `/unity/`로 redirect하고 Unity asset 요청은 SPA fallback으로 rewrite하지 않는다. Vercel preview snapshot은 `Content-Encoding: br` 누락 문제를 피하기 위해 uncompressed WebGL files를 사용한다. |
